...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profileData, error: profileError } = await supabaseClient.from("user_profiles").select("*").eq("user_id", userId).single();
    if (profileError) {
    ...
    if (!profileData || !profileData.linkedin_content && !profileData.additional_details && !profileData.cv_content) {
      ...
      const basicSummary = {
        ...
        overall_blurb: "Add your LinkedIn profile, CV, or additional details to generate a complete professional summary."
      };
...
async function createOrUpdateSummary(userId, summary) {
  const { error } = await supabaseClient.from("user_summaries").upsert({
    user_id: userId,
    ...summary,
    updated_at: new Date().toISOString()
  }, {
    onConflict: "user_id" // This is crucial!
  });
  if (error) {
