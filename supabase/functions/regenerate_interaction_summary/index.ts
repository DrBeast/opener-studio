...
serve(async (req) => {
  ...
  try {
    ...
    if (!interactions || interactions.length === 0) {
      // Update company with no interactions summary
      ...
      return new Response(JSON.stringify({ 
        summary: "No interactions yet with this company.",
...
Keep it concise and actionable.`;
    ...
    const { error: updateError } = await supabase
      .from('companies')
      .update({ interaction_summary: summary.trim() })
      .eq('company_id', companyId)
      .eq('user_id', user.id);
    ...
    console.error('Error regenerating interaction summary:', error);
