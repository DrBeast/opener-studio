...
-- Target criteria specific trigger function
...
-- User feedback specific trigger function
...
CREATE TABLE public.target_criteria (
...
-- Saved Message Versions Table
CREATE TABLE public.saved_message_versions (
    message_version_id uuid NOT NULL DEFAULT uuid_generate_v4(),
    user_id uuid,
    company_id uuid,
    contact_id uuid,
...
-- Saved Message Versions Policies
CREATE POLICY "Users can view their own saved messages" ON public.saved_message_versions
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own saved messages" ON public.saved_message_versions
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own saved messages" ON public.saved_message_versions
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own saved messages" ON public.saved_message_versions
    FOR DELETE USING (auth.uid() = user_id);

-- Target Criteria Policies
CREATE POLICY "Users can view their own target criteria" ON public.target_criteria
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own target criteria" ON public.target_criteria
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own target criteria" ON public.target_criteria
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own target criteria" ON public.target_criteria
    FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can view their own summaries" ON public.user_summaries
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own summaries" ON public.user_summaries
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own summaries" ON public.user_summaries
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own summaries" ON public.user_summaries
    FOR DELETE USING (auth.uid() = user_id);
...
-- Saved message versions triggers
...
CREATE TRIGGER trigger_target_criteria_updated_at
    ...
    FOR EACH ROW
    EXECUTE FUNCTION update_target_criteria_updated_at();
