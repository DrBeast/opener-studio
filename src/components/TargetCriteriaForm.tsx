...
export function TargetCriteriaForm({
  ...
  useEffect(() => {
    if (initialData) {
      setValue(
        "free_form_role_and_company_description",
        initialData.free_form_role_and_company_description || ""
      );
      ...
      if (initialData.similar_companies) {
        setValue(
          "similar_companies",
          Object.keys(initialData.similar_companies).join(", ")
        );
      }
      if (initialData.target_industries) {
        setValue(
          "target_industries",
          Object.keys(initialData.target_industries).join(", ")
        );
      }
      if (initialData.target_locations) {
        setValue(
          "target_locations",
          Object.keys(initialData.target_locations).join(", ")
        );
      }
      if (initialData.target_sizes) {
        setValue(
          "target_sizes",
          Object.keys(initialData.target_sizes).join(", ")
        );
      }
      if (initialData.target_public_private) {
        setValue(
          "target_public_private",
          Object.keys(initialData.target_public_private).join(", ")
        );
      }
      if (initialData.target_functions) {
        setValue(
          "target_functions",
          Object.keys(initialData.target_functions).join(", ")
        );
      }
      if (initialData.target_wfh_preference) {
        setValue(
          "target_wfh_preference",
          Object.keys(initialData.target_wfh_preference).join(", ")
        );
      }
    }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const criteriaData = {
        user_id: user.id,
        free_form_role_and_company_description:
          data.free_form_role_and_company_description,
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Textarea
            ...
            rows={4}
            {...register("free_form_role_and_company_description", {
          ...
          {errors.free_form_role_and_company_description && (
            <p className="text-red-500 text-sm">
              {errors.free_form_role_and_company_description.message}
            </p>
          )}
          <p className="text-sm text-muted-foreground">
            Be specific about role type, company stage, and any other important
            details.
          </p>
        </div>
