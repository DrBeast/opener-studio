...
const ProgressTracker = ({
    ...
    const fetchStatus = async () => {
      ...
      try {
        ...
        const {
          data: profileData,
          error: profileError
        } = await supabase.from("user_profiles").select("linkedin_content, additional_details, cv_content").eq("user_id", user.id).single();
        ...
        const {
          data: targetData,
          error: targetError
        } = await supabase.from("target_criteria").select("*").eq("user_id", user.id).limit(1);
        if (targetError) {
        ...
        const hasBackgroundData = profileData && (profileData.linkedin_content || profileData.additional_details || profileData.cv_content);
        setCompletionStatus({
  ...
  const stepsCompleted = [completionStatus.hasBackground, completionStatus.hasJobTargets].filter(Boolean).length;
  const totalSteps = 2;
  ...
  return <Card className={`bg-primary/5 ${className}`}>
