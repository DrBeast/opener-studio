...
const PersistentAssistant = () => {
  ...
  return <div className={`fixed top-16 bottom-0 right-0 z-40 flex flex-col transition-all duration-300 ${isOpen ? 'w-80' : 'w-12'} border-l shadow-md bg-background`}>
      ...
      {isOpen && <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {messages.map((message, index) => <div key={index} className={`mb-2 ${message.isUser ? "text-right" : ""}`}>
          ...
          <div className="p-2 border-t">
            ...
            {user && <div className="flex flex-wrap gap-1 mb-3">
                <button className="text-xs bg-primary/20 hover:bg-primary/30 text-primary rounded-full px-3 py-1 transition-colors flex items-center" onClick={() => handleNavigationSuggestion("/profile")}>
                  Profile <ArrowRight className="ml-1 h-3 w-3" />
                </button>
                <button className="text-xs bg-primary/20 hover:bg-primary/30 text-primary rounded-full px-3 py-1 transition-colors flex items-center" onClick={() => handleNavigationSuggestion("/job-targets")}>
                  Targets <ArrowRight className="ml-1 h-3 w-3" />
                </button>
                <button className="text-xs bg-primary/20 hover:bg-primary/30 text-primary rounded-full px-3 py-1 transition-colors flex items-center" onClick={() => handleNavigationSuggestion("/companies")}>
            ...
            <div className="flex flex-wrap gap-1 mb-2">
              {suggestions.map((suggestion, index) => <button key={index} className="text-xs bg-primary/10 hover:bg-primary/20 text-primary rounded-full px-2 py-1 transition-colors" onClick={() => handleSuggestionClick(suggestion)}>
                  {suggestion}
                </button>)}
            </div>
            
            <div className="flex">
              <Textarea placeholder="Type a message..." className="min-h-[40px] resize-none" value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyDown={handleKeyPress} />
