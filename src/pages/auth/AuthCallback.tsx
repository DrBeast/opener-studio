    ...
    const handleAuthCallback = async () => {
      try {
        ...
        if (
          ...
          console.log(
            "LinkedIn user detected, metadata structure:",
          ...
          try {
            ...
            if (shouldUpdateProfile) {
              ...
              console.log(
                `Extracted name data from LinkedIn - First: "${firstName}", Last: "${lastName}"`
              );
              ...
              if (firstName || lastName) {
                ...
                if (updateError) {
                  console.error(
                    "Error updating profile with LinkedIn data:",
                    updateError
                  );
                  console.error(
                    "Full error details:",
                  ...
                  if (updateError.code === "42501") {
                    setDebugInfo(
                      "Permission denied. This may be due to Row Level Security (RLS) policies."
                    );
                  } else if (updateError.code === "23505") {
                    setDebugInfo(
                      "Duplicate key violation. The profile may already exist."
                    );
                  }
                } else {
                  console.log(
                    "Profile updated with LinkedIn data successfully"
                  );
                  toast.success(
                    "Profile updated with your LinkedIn information"
                  );
                }
              } else {
                console.log("No name data available from LinkedIn metadata");
                toast.warning(
                  "Could not extract name from your LinkedIn profile"
                );
              }
            } else {
